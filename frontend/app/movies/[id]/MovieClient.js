...
export default function MovieClient({ id, initial }) {
  const movieId = id;
  ...
  const [loading, setLoading] = useState(!(initial && initial.movie));
  ...
  const loadAll = async () => {
    if (!movie) setLoading(true); // already have server-seeded data → refresh silently
    try {
