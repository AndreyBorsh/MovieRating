...
export default function PrivateNote({ mediaType, mediaId, token }) {
  ...
  return (
    <div className="rounded-xl border" style={{ background: "#1b1613", borderColor: "#2e2723" }}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between px-4 py-2.5 text-left group"
      >
        <span className="text-sm text-stone-400 group-hover:text-stone-300 transition-colors flex items-center gap-2">
          📝 Заметка для себя
          {!open && hasNote && (
      ...
      {open && (
        <div className="px-4 pb-4 space-y-2">
          <textarea
            ...
            className="w-full rounded-lg px-3 py-2 text-sm text-stone-200 outline-none focus:ring-1 focus:ring-amber-400/40 resize-y leading-relaxed"
            style={{ background: "#100d0b", border: "1px solid #2e2723", minHeight: 90 }}
          />
