...
export default function TvClient({ id, initial }) {
  const tvId = id;
  const { token, user, ready, logout } = useAuth();

  const [show,     setShow]     = useState(initial?.show || null);
  const [reviews,  setReviews]  = useState(initial?.reviews || []);
  const [myRatings, setMyRatings] = useState([]);
  const [form,     setForm]     = useState(DEFAULT_FORM);
  const [reviewText, setReviewText] = useState("");
  const [loading,  setLoading]  = useState(!(initial && initial.show));
  ...
  const loadAll = async () => {
    if (!show) setLoading(true); // already have server-seeded data → refresh silently
    try {
