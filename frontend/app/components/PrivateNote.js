...
export default function PrivateNote({ mediaType, mediaId, token }) {
  ...
  return (
    <div className="rounded-xl border" style={{ background: "rgba(22,28,52,0.72)", borderColor: "rgba(255,255,255,0.10)" }}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between px-4 py-2.5 text-left group"
      >
        <span className="text-sm text-stone-300 group-hover:text-stone-600 transition-colors flex items-center gap-2">
          📝 Заметка для себя
          {!open && hasNote && (
            <span className="w-1.5 h-1.5 rounded-full bg-red-500/70" title="Есть заметка" />
          )}
        </span>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-2">
          <textarea
            ...
            className="w-full rounded-lg px-3 py-2 text-sm text-stone-100 outline-none focus:ring-1 focus:ring-red-500/40 resize-y leading-relaxed"
            style={{ background: "#141b31", border: "1px solid rgba(255,255,255,0.10)", minHeight: 90 }}
          />
